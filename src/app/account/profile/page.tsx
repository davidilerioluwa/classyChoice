...
const Page = () => {
  ...
  return (
    <div className="w-full text-purple-800 gap-4 h-full pb-12 mt-20 p-4 min-h-screen ">
      ...
      />
      <h1 className="font-bold p-2 text-lg text-purple-800 mb-2">
        Account Profile
      </h1>
      <section className="flex flex-col md:flex-row items-center gap-4 p-4">
        <div className="drop-shadow-md h-80 w-60 border border-purple-100 text-purple-800 p-4  rounded-md flex flex-col gap-2 justify-center items-center">
          ...
          <p className="text-sm">
            {snap.user?.name ? snap.user.name : "click EDIT to update name"}
          </p>
          <p className="text-sm">
            {snap.user?.email ? snap.user.email : "No Email Entered"}
          </p>
          <p className="text-sm">
          ...
          <button
            className="bg-purple-800 rounded-md px-6 py-1 text-white"
        ...
        <div className="drop-shadow-md h-80 w-60 border text-center border-purple-100 text-purple-800 p-4 rounded-md flex flex-col gap-2 justify-center items-center">
          ...
          <p className="text-sm">
            {snap.user?.city ? snap.user.city + ", " : ""}
            {snap.user?.state ? snap.user.state + "." : ""}
          </p>
          <button
            className="bg-purple-800 rounded-md px-6 py-1 text-white"
...
const EditAccountDetails = ({
  ...
  return (
    <section
      className={`text-purple-800 w-full bg-lgray p-4 flex flex-col gap-1 absolute  left-[-100px] z-50 ${
        showEditAccount ? "flex" : "hidden"
      }`}
    >
      <div
        ...
        <div className="relative bg-white   p-8 m-auto w-80 md:w-[400px] rounded-lg  overflow-y-auto">
          ...
          <form className="flex flex-col gap-2 text-sm">
            <div className="flex flex-col gap-1">
              ...
              <input
                disabled
                value={snap.user?.email ? snap.user.email : "No Email Entered"}
                type="text"
...
const EditAddressDetails = ({
  ...
  return (
    <section
      className={`text-purple-800 w-full bg-lgray flex flex-col gap-1 w-[300px] absolute  left-[-100px] z-50 ${
        showEditAddress ? "flex" : "hidden"
      }`}
    >
      <div
        className={`fixed flex items-center justify-center h-screen w-screen bg-black top-0 left-0 z-40 p-4`}
