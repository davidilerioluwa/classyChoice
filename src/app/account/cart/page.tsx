...
const Page = () => {
  ...
  const AddToTotalAmount = ({
    ...
    if (newItem) {
      ...
      {
        !newAmount.find((item) => item.cartId == newItem.cartId) &&
    ...
    <>
      {isLoading ? (
        ...
        <div className="lg:h-screen h-full p-4 pt-24 lg:p-8 lg:pt-24 bg-white">
          {showUpdateProfile && (
            <UpdateProfileForm setShowUpdateProfile={setShowUpdateProfile} />
          )}
          {showPaymentDetails && (
            <PaymentDetails
              postToProducts={postToProducts}
              setShowPaymentDetails={setShowPaymentDetails}
            />
          )}
          <h1 className="font-bold p-2 text-lg text-purple-900 mb-2">
          ...
          <div className="w-full flex flex-col md:flex-row items-center md:items-start  gap-4 h-full pb-12">
            <section className="  bg-white border border-purple-100  drop-shadow-md rounded-md p-2 lg:p-4 text-purple-900 w-full overflow-x-auto">
              {cart.length ? (
                <div className="gap-2 flex flex-col gap-2  md:w-full">
                  ...
                  <div className="flex flex-col gap-2 mb-4 overflow-y-auto">
                    {cart.map((cartItem) => (
                      <CartItem
                        key={cartItem.id}
                        cartItem={cartItem}
                        setPaystackButton={setPaystackButton}
                        getCart={getCart}
            ...
            >
              <div className="w-full my-4">
                <div className="flex justify-between">
                  <span className="text-sm text-purple-900 w-32">
                    Subtotal:
                  </span>
                    ...
                    {addressType == "different" && (
                      <label>
                        <textarea
                          ...
                          className="w-full h-20 px-2 flex py-2 text-purple-900 outline outline-[1px] outline-purple-900 rounded-md"
                        />
                      </label>
                    )}
                  </div>
                ...
                <div>
                  <label className="font-bold text-purple-900 text-sm">
                    Enter note to pass on to the Seller:{" "}
                  </label>
                  <textarea
                    value={note}
                    onChange={(e) => setNote(e.target.value)}
                    placeholder="Note for the seller"
                    className="w-full px-2 flex mt-2 text-purple-900 h-20 outline outline-[1px] outline-purple-900 rounded-md"
                  />
                </div>
              </div>
              {snap.user?.address &&
              ...
              snap.user?.state ? (
                ...
                <button
                  ...
                  className="bg-purple-900 text-white px-4 py-2 rounded-md w-full"
                >
                  Update Profile to proceed to Payment
                </button>
              )}
            </form>
          </div>
