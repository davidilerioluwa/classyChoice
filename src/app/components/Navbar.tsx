...
const Navbar = () => {
  ...
  return (
    <nav className='flex fixed z-30 top-0  justify-between w-screen pl-2 pr-6 md:px-16 py-2 text-lg text-purple-800 bg-white drop-shadow-lg'>
        <div className="flex gap-3 md:gap-4     ">
           <Link href="/" className="hover:underline py-2">Home</Link>
           <div className="py-2 cursor-pointer relative" onMouseEnter={()=> setShowCategoriesDropdown(true)} onMouseLeave={()=> setShowCategoriesDropdown(false)} onClick={()=> setShowCategoriesDropdown(!showCategoriesDropdown)}>
              <span className="flex gap-1 items-center"> <button>Categories</button> <span><FaAngleDown/></span></span>
              {showCategoriesDropdown?<CategoriesDropdown/>:""}
           </div>
        </div>
        <div className='flex items-center gap-3 md:gap-4 text-lg font-bolder'>
            { loggedIn
              ?<span className="py-2 cursor-pointer flex relative" onMouseEnter={()=> setShowAccountDropdown(true)} onMouseLeave={()=> setShowAccountDropdown(false)} onClick={()=> setShowAccountDropdown(!showAccountDropdown)}><FaUserAlt/><FaAngleDown/> {showAccountDropdown?<AccountDropdown/>:""}</span>
              :<Link href={"/api/auth/signin"} className='text-sm py-1.5 border border-purple-800 px-2 md:px-6 rounded-md'>SIGN IN</Link>
            }
...
const AccountDropdown= ()=>{
  ...
  return(<>
  {
    ...
    <div className='absolute top-6 left-[-30px] w-fit bg-white p-4 rounded-md flex flex-col drop-shadow-md text-sm'>
      <Link href={"/dashboard/products"} className='p-2 w-32 text-nowrap hover:drop-shadow-lg rounded-md bg-white'>Products</Link>
      ...
      <Link href={"/dashboard/orders"} className='p-2 text-nowrap hover:drop-shadow-lg rounded-md bg-white'>Orders</Link>
