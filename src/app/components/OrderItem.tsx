...
const OrderItem = ({ item }: { item: product }) => {
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        ...
        setImageUrl(product[0]?.images[0]?.url);
      } catch {}
    };
    getProduct();
  }, []);
  ...
  return (
    <div className="flex  flex-col md:flex-row  items-start md:items-center justify-between md:gap-6 rounded-md  hover:bg-purple-800 hover:text-white px-2 py-1 md:px-6 md:py-2 border border-purple-100 gap-2">
      <div
        key={item.productId}
        className="flex sm:grid grid-cols-12 gap-2  w-full justify-between items-center  rounded-md text-sm  cursor-pointer"
      >
        <div className="h-full col-span-6">
          <div className="flex gap-2 items-center h-full">
            <img
              ...
              height={20}
            />
            <span className="break-words text-wrap w-24 sm:w-32 md:w-48">
              {item.title}
            </span>
          </div>
        </div>
