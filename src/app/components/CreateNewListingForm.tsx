...
const CreateNewListingForm = ({
  ...
  setShowListingForm: Dispatch<SetStateAction<boolean>>;
  EditListingId?: string;
  setEditListingId?: Dispatch<SetStateAction<string>>;
}) => {
  ...
  const addNewVariant = (newVariant: string) => {
    const variationToBeEdited = variations.find(
      (variation) => variation.variation == currentVariation,
    );
  ...
  const updateVariationName = ({
    oldVariationValue,
    newVariationValue,
  }: {
    oldVariationValue: string;
    newVariationValue: string;
  }) => {
    console.log(oldVariationValue, newVariationValue);
    const variationItem = variations.find(
      (variation) => (variation.variation = oldVariationValue),
    );
    if (variationItem) {
  ...
  const updateListing = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    oldUrls.map((oldImg) =>
      formData.append(
        "oldUrls",
        JSON.stringify({ url: oldImg.url, assetId: oldImg.assetId }),
      ),
    );
    ...
    <div className="bg-black z-50 top-0 left-0 bg-opacity-50 fixed flex justify-center items-center h-screen w-screen md:py-40 p-10 md:p-20">
      ...
      <div className="bg-white rounded-md p-4 w-96 h-full md:h-screen  relative overflow-y-auto">
        <span
          ...
          className="absolute top-2 right-2 px-3  pb-1 rounded-md cursor-pointer text-xl  text-purple-900 border border-purple-900 "
        >
          x
        </span>
        <h1 className="text-lg text-center mt-2 font-bold text-purple-800 w-full ">
          Create New Listing
        </h1>
        {isLoading ? (
          ...
          <form
            className="text-purple-800 flex flex-col gap-2"
            onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
              EditListingId ? updateListing(e) : CreateNewListing(e);
            }}
          >
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                type="text"
                className="px-2 py-2 text-purple-800 outline outline-[1px] outline-purple-800 rounded-md"
              />
            </div>
            <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex relative rounded-md justify-between">
                <select
                  ...
                  className="appearance-none  outline-none w-full  border border-purple-800 p-2 py-3 rounded-md text-sm"
                >
            ...
            <div className=" py-2 flex flex-col gap-2">
                  ...
                  >
                    {oldVariationValue ? (
                      <div className="flex w-full justify-between w-full gap-4">
                        <input
                          ...
                          className="px-2 py-2 text-purple-800 outline outline-[1px] outline-purple-800 rounded-md w-full"
                        />
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                    ...
                    <div className="flex flex-wrap gap-2">
                      {variation.variants.map((variant) => (
                        <div
                          key={variant}
                          className="border border-purple-900 px-4 py-0.5 rounded-md flex items-center gap-1"
                        >
                          <span>{variant} </span>
                          <span
                            ...
                            onClick={() => {
                              ...
                              deleteVariant({
                                variantName: variant,
                                currentVariation: variation.variation,
                              });
                            }}
                          >
                            x
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="text-white bg-purple-900 rounded-md py-2 px-4"
                    >
                      +
                    </button>
                  </div>
              ...
              {(showCreateVariation == "variant" ||
                showCreateVariation == "moreVariants") && (
                <div className="flex flex-col gap-2">
              ...
              {(showCreateVariation == "none" ||
                ...
                <button
                  ...
                  className="bg-purple-900 w-full text-white py-2 rounded-md"
                >
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex relative rounded-md justify-between">
                <select
                  ...
                  className="appearance-none  outline-none w-full  border border-purple-800 p-2 py-3 rounded-md text-sm"
                >
                  <option value={"UnLimited Quantity"}>
                    UnLimited Quantity
                  </option>
            ...
            <div>
              ...
              <div className="flex items-center justify-center flex-wrap gap-2 mb-20">
                {oldUrls.map((url, index) => (
                  <div
                    key={url.url}
                    className="relative border border-purple-800 rounded-md"
                  >
                    <span
                      onClick={() => deleteOldUrl(url.assetId)}
                      key={index}
                      className="absolute right-2 top-2 px-3 py-1 cursor-pointer bg-purple-800 text-white rounded-md"
                    >
                      x
                    </span>
                    <img
                ...
                <div className="relative w-32 h-32 cursor-pointer">
                  <input
                    className="w-full bg-purple-800 w-6 h-32 bg-opacity-10 cursor-pointer"
                    type="file"
                    accept="image/*"
                    multiple
                  ...
                  />
                  <div className="w-32 h-32 absolute top-0 right-0 text-white bg-purple-800 flex z-[-10] justify-center items-center text-5xl">
                    <span>+</span>
                  </div>
                </div>
              </div>
            ...
            <button
              type="submit"
              className="w-full rounded-md border border-purple-800 py-2"
            >
