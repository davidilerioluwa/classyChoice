...
const CartItem = ({
  ...
  AddToTotalAmount: ({
    ...
    newItem?: iCheckoutDetails;
    removeItem?: iCheckoutDetails;
  }) => void;
  setPaystackButton: Dispatch<SetStateAction<boolean>>;
}) => {
  ...
  useEffect(() => {
    if (
      ...
      Number(product?.unitsAvailable) > 0
    ) {
  ...
  return (
    <div
      key={cartItem.id}
      className="px-2 py-2 pr-4 w-fit min-w-full md:w-full flex md:grid grid-cols-12 gap-2 justify-between items-center border border-purple-100 rounded-md text-sm hover:bg-purple-900 hover:text-white cursor-pointer"
    >
      <div className="h-full col-span-6 ">
        <div className="flex gap-2 items-center h-full w-20 sm:w-60">
          <img
            ...
            src={
              !isLoading && product == undefined ? "outOfStock.jpeg" : imageUrl
            }
            alt={
              !isLoading && product == undefined ? "outOfStock.jpeg" : "Item"
            }
            className="w-20 h-20 rounded-md border-2"
          />
          <span className=" hidden sm:flex font-bold ">
        ...
        <div className="col-span-5 flex flex-col  gap-2 md:w-full  md:grid grid-cols-5">
          {product?.quantityType == "Limited Quantity" ? (
            <div className=" md:hidden">
              {product?.unitsAvailable} Units left
            </div>
          ) : (
            ""
          )}
          <div className=" h-full col-span-3">
            ...
            Number(product?.unitsAvailable) > 0 ? (
              <div className="flex relative items-center justify-center gap-4 h-full">
                {product?.quantityType == "Limited Quantity" ? (
                  <div className="hidden md:flex md:absolute top-[-25px]">
                    {product?.unitsAvailable} Units left
                  </div>
                ) : (
                  ""
                ...
                <div
                  className="flex justify-center items-center bg-purple-100 text-purple-900 text-lg font-bold h-6 w-6 cursor-pointer rounded-sm "
                ...
                >
                  {" "}
                  <button
                    className={`${product?.quantityType == "Limited Quantity" && product?.unitsAvailable <= Number(cartItem.quantity) && "opacity-50 cursor-not-allowed"}`}
                    disabled={
                      product?.quantityType == "Limited Quantity" &&
                      product?.unitsAvailable <= Number(cartItem.quantity)
                    }
                  >
                    +
                  </button>{" "}
      ...
      <div className="col-span-1 text-right " onClick={() => removeFromCart()}>
        remove
      </div>
    </div>
  );
